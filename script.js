...
function checkQuiz(){
  ...
  if(ok){box.className='feedback show good';box.textContent='Верно! Ты связал каждый рабочий вопрос с подходящим источником данных. Теперь можно завершить курс.';sessionStorage.setItem('sources_quiz','passed');return}
...
addEventListener('DOMContentLoaded',()=>{const url=new URL(location.href);if(url.searchParams.get('reset')==='1'){localStorage.removeItem(unlockKey);localStorage.removeItem('restaurant_sources_completed');sessionStorage.removeItem('sources_quiz');sessionStorage.removeItem('sources_quiz_attempts');unlocked=0;url.searchParams.delete('reset');history.replaceState({},'',url.pathname+(url.hash||''))}applyLocks();const saved=localStorage.getItem('restaurant_sources_completed')==='passed';if(saved){document.getElementById('complete').disabled=true;document.getElementById('complete').textContent='Завершено';document.getElementById('completion-title').textContent='Курс пройден';document.getElementById('completion-text').textContent='Результат сохранён. Теперь ты знаешь, где искать показатели и детали процессов.'}const requested=location.hash.slice(1);go(pages.includes(requested)&&pages.indexOf(requested)<=unlocked?requested:'home')});
